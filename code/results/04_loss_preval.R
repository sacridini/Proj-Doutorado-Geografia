...
preval <- terra::rast("/media/elacerda/backup/doutorado_backup/raster/ltgee_loss_greatest_2018_seg6/mosaics/mosaic_clip_preval.tif")
...
for (r in names(bin_rasters)) {
  ...
  terra::writeRaster(result, paste0("~/doutorado/testes/loss/finais/preval/", names(bin_rasters[[r]]), "_preval.tif"), wopt=list(gdal=c("COMPRESS=DEFLATE")))
}